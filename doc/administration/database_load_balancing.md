...
This is a structured log, where each line is a JSON object, carrying at least an
`event` field (useful for filtering) and a human readable `message` field, some event-specific metadata (e.g. `db_host`), as well as some contextual information that is always logged (e.g. `severity` and `time`). For example:
