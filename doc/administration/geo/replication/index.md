...
The following table list the GitLab features and which ones are or not replicated, and verified on a **secondary** node. You can keep track of the progress to include the missing items in [ee-893], and [ee-1430].
...
| Personal snippets (after [version control for snippets](https://gitlab.com/gitlab-org/gitlab-ce/issues/13426)) | No | No |
| Project snippets | Yes | Yes |
| Project snippets (after [version control for snippets](https://gitlab.com/gitlab-org/gitlab-ce/issues/13426)) | No | No |
...
| [Container Registry](../../container_registry.md) | No | No |
...
| [Content in object storage](https://gitlab.com/groups/gitlab-org/-/epics/1526) | No | No |

1. The integrity can be verified manually by following [these instructions](https://docs.gitlab.com/ee/administration/raketasks/check.html) on both nodes and comparing the output between them.

DANGER: **DANGER**
Feature not on this list, or with a **No** on the **Replicated** column is unavailable on the **secondary** node. Failing over without manually replicating data not on this list will cause the data to be **lost**. If you wish to use them on a **secondary** node, or to execute a failover successfully, you will need to replicate their data using some other means.
