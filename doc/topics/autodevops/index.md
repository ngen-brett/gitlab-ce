...
Note that even when disabled at the instance level, group owners and project maintainers are still able to enable
...
Note that only administrators and group owners can enable or disable Auto DevOps at the group level.
...
This can be enabled from the console with the following, which contains the examlpe of 10%:
