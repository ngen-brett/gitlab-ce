...
GitLab uses `elasticsearch-rails` for handling communication with Elasticsearch server. However, in order to achieve zero-downtime deployment during schema changes, an extra abstraction layer is build to allow:

* Indexing (writes) to multiple indexes, with different mappings
* Switching to different index for searches (reads) on the fly

Currently we are on the process of migrating models to this new architecture (e.g. `Snippet`). The new design is hardwired to work with a single version for now.
...
1. Change the namespace for files under `v12p3` from `V12p1` to `V12p3` (which are still alias to `Latest`)
1. Delete `v12p1` folder
1. Copy the entire folder of `latest` as `v12p1`
1. Change the namespace for files under `v12p1` from `Latest` to `V12p1`
