...
The amount of precision that time has when returned from a database
can differ from time objects in Ruby, so we need tolerances when
comparing in specs. We can use `be_like_time` to compare that times are
