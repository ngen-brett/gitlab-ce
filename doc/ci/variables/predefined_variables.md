...
| `CI_MERGE_REQUEST_SOURCE_BRANCH_SHA`    | 11.9   | all    | The HEAD sha of the source branch of the merge request if [the pipelines are for merge requests](../merge_request_pipelines/index.md) and [pipelines for merged results](../merge_request_pipelines/pipelines_for_merged_results/index.md) enabled. Available only if `only: [merge_requests]` is used and the merge request is created.  |
