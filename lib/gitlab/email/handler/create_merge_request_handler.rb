  ...
  module Email
    module Handler
      class CreateMergeRequestHandler < BaseHandler
        ...
        HANDLER_REGEX        = /\A.+-(?<project_id>.+)-(?<authtoken>.+)-merge-request\z/.freeze
        HANDLER_REGEX_LEGACY = /\A(?<project_path>[^\+]*)\+merge-request\+(?<authtoken>.*)/.freeze

        def initialize(mail, mail_key)
          super(mail, mail_key)

          if m = HANDLER_REGEX.match(mail_key.to_s)
            @project_id, @incoming_email_token = m.captures
          elsif m = HANDLER_REGEX_LEGACY.match(mail_key.to_s)
