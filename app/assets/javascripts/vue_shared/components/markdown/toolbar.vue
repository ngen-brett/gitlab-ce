  ...
  <div class="comment-toolbar clearfix">
    <div class="toolbar-text">
      <template v-if="!hasQuickActionsDocsPath && markdownDocsPath">
        <gl-link :href="markdownDocsPath" target="_blank" tabindex="-1"
          >{{ __("Markdown is supported") }}</gl-link
        >
      </template>
      <template v-if="hasQuickActionsDocsPath && markdownDocsPath">
        <gl-link :href="markdownDocsPath" target="_blank" tabindex="-1">{{ __("Markdown") }}</gl-link> and
        <gl-link :href="quickActionsDocsPath" target="_blank" tabindex="-1">{{ __("quick actions") }}</gl-link>
        are supported
      </template>
    </div>
    <span v-if="canAttachFile" class="uploading-container">
      ...
      <span class="uploading-error-container hide">
        ...
        <button class="attach-new-file markdown-selector" type="button">{{ __("attach a new file") }}</button>
      </span>
      ...
      <button class="btn btn-default btn-sm hide button-cancel-uploading-files" type="button">{{ __("Cancel") }}</button>
    </span>
  </div>
</template>
