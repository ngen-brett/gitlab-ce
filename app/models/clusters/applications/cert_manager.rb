  ...
  module Applications
    class CertManager < ApplicationRecord
      ...
      def private_key_name
        cluster_issuer_content.dig('spec', 'acme', 'privateKeySecretRef', 'name')
      end

      def delete_private_key
        "kubectl delete secret -n #{Gitlab::Kubernetes::Helm::NAMESPACE} #{private_key_name}" if private_key_name.present?
      end

      def delete_crd(definition)
        "kubectl delete crd #{definition}"
      end
