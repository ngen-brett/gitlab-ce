...
class KubernetesService < DeploymentService
  ...
  def predefined_variables(project:)
    Gitlab::Ci::Variables::Collection.new.tap do |variables|
      variables
        ...
        .append(key: 'KUBE_NAMESPACE', value: namespace_for(project))
  ...
  def terminals(environment)
    with_reactive_cache do |data|
      ...
      terminals = pods.flat_map { |pod| terminals_for_pod(api_url, namespace_for(project), pod) }.compact
  ...
  def kubeconfig
    to_kubeconfig(
      url: api_url,
      namespace: namespace_for(project),
      token: token,
      ca_pem: ca_pem)
  end
  ...
  def build_kube_client!
    raise "Incomplete settings" unless api_url && namespace_for(project) && token
  ...
  def read_pods
    ...
    kubeclient.get_pods(namespace: namespace_for(project)).as_json
