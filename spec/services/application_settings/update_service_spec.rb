  ...
  describe 'updating outbound_local_requests_whitelist' do
    context 'when param outbound_local_requests_whitelist is blank' do
      ...
      it 'does not add to whitelist' do
        expect { subject.execute }.not_to change { application_settings.outbound_local_requests_whitelist }
      end
    end
    ...
    context 'when param outbound_local_requests_whitelist contains values' do
      ...
      it 'adds to whitelist' do
        expect { subject.execute }.to change { application_settings.outbound_local_requests_whitelist }
        expect(application_settings.outbound_local_requests_whitelist).to contain_exactly('127.0.0.1', 'example.com')
      end
    end
    ...
    context 'when param outbound_local_requests_whitelist_raw is passed' do
      ...
      it 'overwrites the existing whitelist' do
        expect { subject.execute }.to change { application_settings.outbound_local_requests_whitelist }
        expect(application_settings.outbound_local_requests_whitelist).to contain_exactly('example.com', 'gitlab.com')
      end
    end
  end
