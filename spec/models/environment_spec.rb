...
describe Environment, :use_clean_rails_memory_store_caching do
  ...
  describe '#terminals' do
    ...
    context 'reactive cache has pod data' do
      ...
      before do
        stub_reactive_cache(environment, pods: pods)
        # allow(environment).to receive(:deployment_platform).and_return(cluster)
      end

      it 'retrieves terminals from the deployment platform' do
        expect(environment.deployment_platform)
          .to receive(:terminals).with(environment, pods)
  ...
  describe '#calculate_reactive_cache' do
    ...
    it 'returns pod data from the deployment platform' do
      expect(environment.deployment_platform).to receive(:read_pods).and_return(%w(pod1 pod2))
