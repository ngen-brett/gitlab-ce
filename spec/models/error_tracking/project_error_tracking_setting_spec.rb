...
describe ErrorTracking::ProjectErrorTrackingSetting do
  ...
  describe '#list_sentry_issues' do
    ...
    context 'when sentry client raises Sentry::Client::SentryError' do
      let(:sentry_client) { spy(:sentry_client) }
      ...
      before do
        ...
        allow(subject).to receive(:sentry_client).and_return(sentry_client)
        allow(sentry_client).to receive(:list_issues).with(opts)
          .and_raise(Sentry::Client::SentryError, 'Sentry API response is missing keys. key not found: "id"')
      end
