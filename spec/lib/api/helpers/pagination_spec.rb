...
describe API::Helpers::Pagination do
  ...
  describe '#paginate (keyset pagination)' do
    ...
    let(:base_query) { { pagination: 'keyset', foo: 'bar', bar: 'baz' } }
    let(:query) { base_query }

    before do
      allow(subject).to receive(:header).and_return(value)
      allow(subject).to receive(:params).and_return(query)
      allow(subject).to receive(:request).and_return(double(url: "#{Gitlab.config.gitlab.url}:8080/api/v4/projects?#{query.to_query}"))
    end
    ...
    context 'when resource can be paginated' do
      ...
      describe 'first page' do
        ...
        it 'adds appropriate headers' do
          ...
          expect_header('X-Next-Page', "#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(ks_prev_id: projects[1].id).to_query}")
      ...
      describe 'second page' do
        ...
        it 'adds appropriate headers' do
          expect_header('X-Per-Page', '2')
          expect_header('X-Next-Page', "#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(ks_prev_id: projects[2].id).to_query}")
        ...
        context 'is present' do
          ...
          describe 'second page' do
            ...
            it 'it returns the right link to the next page' do
              expect_header('X-Per-Page', '2')
              expect_header('X-Next-Page', "#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(ks_prev_id: projects[6].id, ks_prev_name: projects[6].name).to_query}")
              expect_header('Link', anything) do |_key, val|
                expect(val).to include('rel="next"')
              end
      ...
      describe 'first page' do
        shared_examples 'response with pagination headers' do
          it 'adds appropriate headers' do
            ...
            expect_header('Link', anything) do |_key, val|
              expect(val).to include(%Q(<#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(page: 1).to_query}>; rel="first"))
              expect(val).to include(%Q(<#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(page: 2).to_query}>; rel="last"))
              expect(val).to include(%Q(<#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(page: 2).to_query}>; rel="next"))
      ...
      describe 'second page' do
        ...
        it 'adds appropriate headers' do
          ...
          expect_header('Link', anything) do |_key, val|
            expect(val).to include(%Q(<#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(page: 1).to_query}>; rel="first"))
            expect(val).to include(%Q(<#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(page: 2).to_query}>; rel="last"))
            expect(val).to include(%Q(<#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(page: 1).to_query}>; rel="prev"))
    ...
    context 'when resource empty' do
      describe 'first page' do
        ...
        it 'adds appropriate headers' do
          ...
          expect_header('Link', anything) do |_key, val|
            expect(val).to include(%Q(<#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(page: 1).to_query}>; rel="first"))
            expect(val).to include(%Q(<#{Gitlab.config.gitlab.url}/api/v4/projects?#{query.merge(page: 1).to_query}>; rel="last"))
            expect(val).not_to include('rel="prev"')
            expect(val).not_to include('rel="next"')
