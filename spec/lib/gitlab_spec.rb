...
describe Gitlab do
  ...
  describe '.com_or_org?' do
    it 'is true when on .com' do
      allow(described_class).to receive(:com?).and_return(true)
      allow(described_class).to receive(:org?).and_return(false)

      expect(described_class.com_or_org?).to eq true
    end

    it 'is true when org' do
      allow(described_class).to receive(:org?).and_return(true)
      allow(described_class).to receive(:com?).and_return(false)

      expect(described_class.com_or_org?).to eq true
    end

    it 'is false when not org or com' do
      allow(described_class).to receive(:org?).and_return(false)
      allow(described_class).to receive(:com?).and_return(false)

      expect(described_class.com_or_org?).to eq false
    end
  end
