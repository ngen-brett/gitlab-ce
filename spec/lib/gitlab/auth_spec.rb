  ...
  describe 'find_for_git_client' do
    context 'build token' do
      ...
      context 'for running build' do
        ...
        before do
          expect(gl_auth).to_not receive(:rate_limit!).with('ip', success: true, login: 'gitlab-ci-token')
        end
      ...
      (HasStatus::AVAILABLE_STATUSES - ['running']).each do |build_status|
        context "for #{build_status} build" do
          ...
          before do
            expect(gl_auth).to_not receive(:rate_limit!).with('ip', success: false, login: 'gitlab-ci-token')
          end
