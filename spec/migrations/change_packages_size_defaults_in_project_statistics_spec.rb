...
describe ChangePackagesSizeDefaultsInProjectStatistics, :migration do
  ...
  it 'defaults packages_size to 0' do
    ...
    p = projects.create!(name: 'a new project', namespace_id: 2)
    stat = project_statistics.create!(project_id: p.id, namespace_id: p.namespace_id)
    expect(ProjectStatistics.find(stat.id).packages_size).to eq(0)
  end
end
