...
describe API::Projects do
  ...
  describe 'POST /projects/:id/fork' do
    ...
    before do
      ...
      project.add_reporter(user3)

      project2.add_reporter(user2)
      project2.add_reporter(user3)
    end
    ...
    context 'when authenticated' do
      ...
      it 'accepts a path for the target project' do
        post api("/projects/#{project.id}/fork", user2), params: { path: 'foobar', namespace: user2.namespace.id }
      ...
      it 'fails to fork if path is already taken' do
        post api("/projects/#{project2.id}/fork", user2), params: { path: 'foobar', namespace: user2.namespace.id }
      ...
      it 'accepts a name for the target project' do
        post api("/projects/#{project.id}/fork", user3), params: { name: 'My Random Project', namespace: user3.namespace.id }

        expect(response).to have_gitlab_http_status(201)
        expect(json_response['name']).to eq('My Random Project')
        expect(json_response['path']).to eq(project.path)
        expect(json_response['owner']['id']).to eq(user3.id)
        expect(json_response['namespace']['id']).to eq(user3.namespace.id)
        expect(json_response['forked_from_project']['id']).to eq(project.id)
        expect(json_response['import_status']).to eq('scheduled')
        expect(json_response).to include("import_error")
      end
