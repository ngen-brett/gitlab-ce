...
describe('RelatedIssuableItem', () => {
  ...
  it('contains issuable-info-container class when canReorder is false', () => {
    expect(wrapper.props('canReorder')).toEqual(false);
    expect(wrapper.find('.issuable-info-container').exists()).toEqual(true);
  });

  it('does not render token state', () => {
    expect(wrapper.find('.text-secondary svg').exists()).toEqual(false);
  });

  it('does not render remove button', () => {
    expect(wrapper.find({ ref: 'removeButton' }).exists()).toEqual(false);
  });
  ...
  describe('token title', () => {
    ...
    it('renders confidential icon', () => {
      expect(wrapper.find('.confidential-icon').exists()).toEqual(true);
    });
  ...
  describe('token state', () => {
    ...
    it('renders if hasState', () => {
      expect(tokenState.exists()).toEqual(true);
    });
    ...
    it('renders open icon when open state', () => {
      expect(tokenState.classes('issue-token-state-icon-open')).toEqual(true);
    });
    ...
    it('renders close icon when close state', done => {
      ...
      Vue.nextTick(() => {
        expect(tokenState.classes('issue-token-state-icon-closed')).toEqual(true);
  ...
  describe('token metadata', () => {
    ...
    it('renders due date component', () => {
      expect(tokenMetadata.find('.js-due-date-slot').exists()).toEqual(true);
    });

    it('renders weight component', () => {
      expect(tokenMetadata.find('.js-weight-slot').exists()).toEqual(true);
    });
  });
  ...
  describe('remove button', () => {
    ...
    it('triggers onRemoveRequest when clicked', () => {
      ...
      expect(wrapper.emitted().relatedIssueRemoveRequest[0]).toEqual([props.idKey]);
    });
  });
});
