    ...
    context 'when the sha parameter matches the source SHA' do
      ...
      context 'when a squash commit message is passed' do
        ...
        it 'passes the same message to SquashService' do
          ...
          expect_next_instance_of(MergeRequests::SquashService, project, user, params) do |squash_service|
            expect(squash_service).to receive(:execute).with(merge_request).and_return({
              status: :success,
